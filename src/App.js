/**
 * src/App.js — Main shell. Handles navigation and keepalive.
 */
...
const CSS = `
  ...
  *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
  ...
  .mode-card { transition:transform .2s, border-color .2s; }
  .mode-card:hover { transform:translateY(-2px); }
`;

// ── Screen wrapper with back button ───────────────────────────────────────────
        ...
        <div style={{ flex:1 }}>
          ...
          {tags && (
            <div style={{ display:"flex", flexWrap:"wrap", gap:5 }}>
              {tags.map(t => <span key={t} style={{ fontSize:10, padding:"2px 8px", borderRadius:20, background:`${accent}14`, color: accent }}>{t}</span>)}
            </div>
          )}
        </div>
...
export default function App() {
  const [screen, setScreen] = useState("home"); // home | gamedle | infinite | speedrun
  ...
  if (screen === "gamedle") return (
    <ScreenWrap onBack={() => setScreen("home")} label="🏆 GAMEDLE" accent="#f0c030">
      <Gamedle onBack={() => setScreen("home")} />
    </ScreenWrap>
  );

  if (screen === "infinite") return (
    <ScreenWrap onBack={() => setScreen("home")} label="♾️ INFINITE" accent="#9b87f8">
      <Infinite onBack={() => setScreen("home")} defaultTab="normal" />
    </ScreenWrap>
  );

  if (screen === "speedrun") return (
    <ScreenWrap onBack={() => setScreen("home")} label="⚡ SPEEDRUN" accent="#e09070">
      <Infinite onBack={() => setScreen("home")} defaultTab="speedrun" />
    </ScreenWrap>
  );
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#080810", fontFamily:"'Segoe UI',system-ui,sans-serif", padding:"0 20px 40px" }}>
      ...
      {/* Hero */}
      <div style={{ maxWidth:520, margin:"0 auto", paddingTop:48, paddingBottom:36, textAlign:"center", animation:"fadeUp .5s ease both" }}>
      ...
      <div style={{ maxWidth:520, margin:"0 auto", display:"flex", flexDirection:"column", gap:10 }}>
        ...
        <div style={{ animation:"fadeUp .5s ease .05s both" }}>
          <ModeCard
            icon="🏆" title="GAMEDLE" badge="DAILY" badgeColor="#f0c030" accent="#f0c030"
        ...
        <div style={{ animation:"fadeUp .5s ease .1s both" }}>
          <ModeCard
            icon="♾️" title="INFINITE" accent="#9b87f8"
            desc="Endless games with no time pressure. Pick a genre category, choose your difficulty, and chase the highest streak."
        ...
        <div style={{ animation:"fadeUp .5s ease .15s both" }}>
          <ModeCard
            icon="⚡" title="SPEEDRUN" accent="#e09070"
            desc="Race against the clock — how many games can you guess correctly in 30 seconds, 1 minute, or 5 minutes? Choose your game pool from Top 10 to Top 250."
      ...
      {/* Footer */}
      <p style={{ textAlign:"center", fontSize:11, color:"#1a1a2a", marginTop:28 }}>
        Powered by IGDB · {new Date().toLocaleDateString()}
      </p>
    </div>
  );
}
