/**
 * src/Infinite.js — Infinite mode + Speedrun mode.
 */
...
const MOCK_GAMES = [
  ...
  { id:4,  title:"Minecraft",                cover:"https://images.igdb.com/igdb/image/upload/t_cover_big/co49x5.jpg", year:2011, genre:"Sandbox",          studio:"Mojang Studios",     hints:["This is a Sandbox game.","It was first released in 2011.","It was developed by Mojang Studios.","This game is one of the best-selling games of all time."],   aliases:["mine craft","mc"] },
  ...
  { id:6,  title:"Portal 2",                 cover:"https://images.igdb.com/igdb/image/upload/t_cover_big/co1x7d.jpg", year:2011, genre:"Puzzle",           studio:"Valve",              hints:["This is a Puzzle game.","It was first released in 2011.","It was developed by Valve.","This game is a co-op puzzle-platform game."],                          aliases:["portal 2","portal2","portal"] },
];

const DIFF_CFG = {
  easy:   { label:"Easy",   emoji:"🟢", blurStart:8,  blurStep:3, blurMin:0, grayscale:false, freeHints:0, scoreMult:0.75, minRatings:50  },
  medium: { label:"Medium", emoji:"🟡", blurStart:14, blurStep:4, blurMin:0, grayscale:false, freeHints:0, scoreMult:1.0,  minRatings:10  },
  hard:   { label:"Hard",   emoji:"🔴", blurStart:22, blurStep:6, blurMin:3, grayscale:true,  freeHints:1, scoreMult:1.5,  minRatings:3   },
};
...
function Leaderboard({ rows, accent="#9b87f8", valueFmt, myUsername, personalBest, pbLabel }) {
  const medals = ["🥇","🥈","🥉"];
  ...
  return (
    <div style={{ border:"1px solid #1e1e2e", borderRadius:12, overflow:"hidden" }}>
      ...
      {myIdx===-1 && myUsername && personalBest && (
        <div style={{ borderTop:"1px solid #1e1e2e", padding:"7px 12px", fontSize:12, display:"flex", justifyContent:"space-between", background:`${accent}08` }}>
...
export default function Infinite({ onBack, defaultTab="normal" }) {
  ...
  const loadLeaderboard = useCallback(async () => {
    const [lb, pb] = await Promise.all([
      getLeaderboard(tab==="speedrun"
        ? { mode:"speedrun", timeLimit:srTime, limit:10 }
        : { mode:"infinite", difficulty:diff,  limit:10 }),
      getPersonalBest(userId,
        tab==="speedrun"?"speedrun":"infinite",
        tab==="speedrun"?{ timeLimit:srTime }:{ difficulty:diff }),
    ]);
  ...
  const loadGames = useCallback(async (d, cat, seen, minRatOverride) => {
    setLoading(true);
    try {
      const games   = await fetchGames(d, cat, seen, minRatOverride);
      const newSeen = [...new Set([...seen, ...games.map(g => g.id)])];
      setSeenIds(newSeen);
      setQueue(prev => phase==="setup" ? shuffle(games) : [...prev.slice(0, idx+1), ...shuffle(games)]);
      setUsingMock(false);
    } catch (err) {
      console.warn("[Infinite] IGDB fetch failed:", err.message, "— using mock");
      setUsingMock(true);
  ...
  function startGame() {
    setScore(0); setStreak(0); setBest(0); setIdx(0); setSeenIds([]); setPhase("playing");
    if (tab==="speedrun") {
      setSrCorrect(0); srCorrectRef.current=0; setTimeLeft(srTime);
      startSpeedrunBG(); // ramp up tempo
    } else {
      startBG(); // normal tempo
    }
    const poolCfg = SR_POOLS.find(p => p.value===srPool);
    loadGames(diff, category, [], tab==="speedrun" ? poolCfg?.minRatings : null);
  }

  // Speedrun countdown + red flash
  useEffect(() => {
    ...
    timerRef.current = setInterval(() => {
      setTimeLeft(t => {
        if (t <= 1) {
          ...
          stopSpeedrunBG();
          const correct    = srCorrectRef.current;
          const finalScore = Math.round(correct*(SR_DIFF_MULT[diff]??1)*100);
          setPhase("done");
          saveScore({ userId, username, mode:"speedrun", score:finalScore, difficulty:diff, gamesCorrect:correct, timeLimit:srTime });
          loadLeaderboard();
          return 0;
        }
        return t - 1;
  ...
  function calcScore(att) {
    ...
    return Math.round(base * cfg.scoreMult * mult);
  }

  function handleWin(att, isCheat) {
    if (isCheat) return;
    setScore(s => s + calcScore(att));
  ...
  async function handleEndSession() {
    stopSpeedrunBG(); // always safe — resets ramp if in speedrun
      ...
      {tab==="speedrun" && (
        <>
          ...
          <div style={{ marginBottom:10, padding:"8px 12px", borderRadius:8, background:"rgba(224,112,48,.06)", border:"1px solid rgba(224,112,48,.15)", fontSize:11, color:"#8a5030" }}>
            ⚡ Hard = 2× points · Medium = 1.5× · Easy = 1×
          </div>
        </>
      )}
      <Leaderboard rows={leaderboard} myUsername={username} personalBest={personalBest}
        accent={tab==="speedrun"?"#e09070":"#9b87f8"}
        valueFmt={tab==="speedrun" ? r=>`${r.games_correct} games (${r.difficulty})` : r=>r.score.toLocaleString()+" pts"}
        pbLabel={personalBest ? (tab==="speedrun" ? `${personalBest.games_correct} games (${personalBest.difficulty})` : personalBest.score.toLocaleString()+" pts") : null}
      />
  ...
  if (phase==="done" && tab==="speedrun") {
    ...
    return (
      <div style={{ maxWidth:520, margin:"0 auto", textAlign:"center" }}>
        ...
        <Leaderboard rows={leaderboard} myUsername={username} personalBest={personalBest} accent="#e09070"
        ...
        <div style={{ display:"flex", gap:8, marginTop:16 }}>
          <button onClick={()=>{ stopSpeedrunBG(); setPhase("setup"); }} style={{ flex:1, padding:"12px", borderRadius:10, border:"1px solid #2a2a40", background:"#0e0e1c", color:"#8b8baa", cursor:"pointer", fontWeight:600 }}>⚙️ Settings</button>
        ...
        <button onClick={()=>{ stopSpeedrunBG(); onBack(); }} style={{ width:"100%", padding:"10px", borderRadius:10, border:"none", background:"none", color:"#3a3a5a", cursor:"pointer", marginTop:8, fontSize:12 }}>← Back to menu</button>
      </div>
    );
  }
  ...
  return (
    <div style={{ maxWidth:520, margin:"0 auto", position:"relative" }}>
      ...
      {/* Red flash overlay during final countdown */}
      {isLowTime && (
      ...
      <div style={{ display:"flex", gap:14, alignItems:"center", justifyContent:"space-between", marginBottom:14, background:"#0a0a18", borderRadius:12, padding:"10px 14px" }}>
        <div style={{ display:"flex", gap:14 }}>
          {[["SCORE",score.toLocaleString()],["STREAK",`${streak}🔥`],["BEST",`${best}⭐`],
            streak>=10?["MULT","3×⚡"]:streak>=5?["MULT","2×⚡"]:streak>=3?["MULT","1.5×⚡"]:null
      ...
      <GameBoard
        ...
        onWin={handleWin} onLose={handleLose} onReadyForNext={handleNext}
      />

      {tab==="normal" && (
        <div style={{ textAlign:"center", marginTop:16 }}>
          <div style={{ fontSize:11, color:"#2a2a40", marginBottom:6 }}>
            Game {idx+1} of {queue.length} · {CATEGORIES[category]?.label??"Random"} · {usingMock?"Mock":"IGDB"}
          </div>
